...
class SDTAEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(self.gamma_xca * self.xca(self.norm_xca(x))) # XCA Attention
...
'''
Juan's
Go back to regular self-attention where the context is a spatial sided square matrix
instead of channels
'''
class XSA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
