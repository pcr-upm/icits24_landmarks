...
from ..layers import LandmarksLayer
...
class Attention4D(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        k = self.k(x).flatten(2).reshape(B, self.num_heads, -1, self.N).permute(0, 1, 2, 3) # Transpose
        v = self.v(x)
...
class EfficientFormerV2(nn.Module):
    def __init__(self, layers, embed_dims=None,
        ...
        if self.fork_feat:
            ...
            self.head = LandmarksLayer(embed_dims[-1], num_landmarks) 
            self.dist = distillation
            if self.dist:
                self.dist_head = LandmarksLayer(embed_dims[-1], num_landmarks) 
...
def _cfg(url='', **kwargs):
    return {
        'url': url, # TODO - 'input_size': (3, 224, 224)
